...
$curlConnectionInitialization = curl_init("https://" . $APIurl . "/youtube/v3/videos?part=snippet&chart=mostPopular&maxResults=" . $MaxCount . "&type=video&regionCode=". $_GET["region"] . "&key=" . $APIkey);
...
if($kindResponse == "youtube#videoListResponse"){
	...
	$description = htmlspecialchars($decodeResponce['items'][$i]['snippet']['description']);
...
<yt:hd />
    <media:group>
      ...
      <media:content url="http://$baseURL/$videoID.3gpp" type="video/3gpp" medium="video" expression="full" duration="$durationInSeconds" yt:format="2" />
      ...
      <media:content url="http://$baseURL/$videoID.3gpp" type="video/3gpp" medium="video" expression="full" duration="$durationInSeconds" yt:format="9" />
...
xmlns:yt='http://$baseURL/schemas/2007'>
    ...
    <generator version='2.0' uri='http://$baseURL/'>YouTube data API</generator>
